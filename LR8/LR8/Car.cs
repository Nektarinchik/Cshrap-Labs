...
namespace Transport
{
    class Car : Vehicle, IComparable<Car>
    {
        public Car(Сolors color, EngineTypes engineType, FuelTypes fuelType, BodyworkTypes bodyWork,
            ...
            int yearOfRelease, int price, int mileage, int seatsAmount) : base(color, engineType, fuelType, engineCapacity, weight,
            fuelСonsumption, yearOfRelease, price, mileage)
        {
            if (carsAmount == Int32.MaxValue)
                throw new ArgumentOutOfRangeException("too many cars");
            Number = carsAmount;
            ++carsAmount;
        ...
        protected static int carsAmount = 0;
    }
}